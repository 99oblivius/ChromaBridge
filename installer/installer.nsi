...
!define MAIN_APP_EXE "color-interlacer.exe"
...
Section -MainProgram
    ...
    ; Main files
    File "..\target\release\${MAIN_APP_EXE}"
...
Section "Start Menu Shortcuts" SecStartMenu
    ...
    CreateShortcut "$SMPROGRAMS\$StartMenuFolder\${APP_NAME}.lnk" "$INSTDIR\${MAIN_APP_EXE}"
...
Function .onInit
    ...
    ; Check if main app or overlay is running using tasklist
    StrCpy $IsAppRunning "0"

    nsExec::ExecToStack 'tasklist /FI "IMAGENAME eq ${MAIN_APP_EXE}" /NH'
    Pop $R0 ; Return value
    Pop $R1 ; Output
    ${If} $R0 == 0
...
Function LeaveRunningAppPage
    ${If} $IsAppRunning == "1"
        ...
        nsExec::Exec 'taskkill /F /IM "${MAIN_APP_EXE}"'
